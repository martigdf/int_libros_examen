...
import { BookPostSchema, BookSchema } from '../../schemas/book/bookSchema.js';
const bookRoutes = async (fastify, opts) => {
    fastify.get('/', {
        ...
        handler: async (request, reply) => {
            const res = await query(`SELECT * FROM libros'`);
            if (res.rowCount === 0) {
    ...
    fastify.get('/:id', {
        ...
        handler: async (request, reply) => {
            ...
            const res = await query(`SELECT * FROM libros WHERE id = $1`, [id]);
            if (res.rowCount === 0) {
    ...
    fastify.get('/my-books', {
        ...
        handler: async (request, reply) => {
            ...
            const res = await query(`SELECT books.*
        FROM books
        INNER JOIN publications ON books.id = publications.id_book
        WHERE publications.id_user = $1`, [user.id]);
            if (res.rowCount === 0) {
