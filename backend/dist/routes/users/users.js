...
import { UserIdSchema, UserSchema, UserPostSchema } from '../../schemas/user/userSchema.js';
import { RequestBaseSchema } from '../../schemas/requests/requestSchema.js';
import bcrypt from 'bcryptjs';
const usersRoute = async (fastify, opts) => {
    fastify.get('/:id', {
        schema: {
            tags: ['users'],
    ...
    fastify.get('/:id/sent-requests', {
        ...
        handler: async function (request, reply) {
            const { id } = request.params;
            const res = await query(`SELECT * FROM requests WHERE requester_user_id = $1`, [id]);
            if (res.rowCount === 0) {
    ...
    fastify.get('/:id/received-requests', {
        ...
        handler: async function (request, reply) {
            const { id } = request.params;
            const res = await query(`SELECT * FROM requests WHERE receiver_user_id = $1`, [id]);
            if (res.rowCount === 0) {
    ...
    fastify.post('/register', {
        ...
        handler: async function (request, reply) {
            ...
            try {
                const res = await query(`INSERT INTO users (name, lastname, username, email, password, role) 
            VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, [name, lastname, username, email, hashedPassword, role]);
                if (res.rowCount === 0) {
